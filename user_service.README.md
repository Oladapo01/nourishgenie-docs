...
## 2\. Features
  ...
  * **Input Validation & Field Whitelisting**: Client-supplied data is validated for type and, where applicable, bounded range before being written to the database — for example, nutrition goal values are range-checked and unknown fields in a profile update are rejected rather than silently accepted or written through unchecked.
...
#### `PUT /profile`
  ...
  * **Errors**: `400 Bad Request` (no data provided, an unrecognized field in the body or in `preferences`, or a value that fails validation — see note below), `401 Unauthorized`, `500 Internal Server Error`.
  * **Note**: Unknown top-level fields and unknown `preferences` fields are rejected rather than silently ignored. `nutritionGoals` submitted here is validated against the same bounds as `PUT /nutrition-goals` (see Section 7) — previously this endpoint wrote whatever was submitted straight to the database with no validation at all, so the range checks enforced on the dedicated goals endpoint could be bypassed entirely by going through `/profile` instead.
...
#### `POST /food-entries`
  ...
  * **Response (201 Created)**:
    ...
    ```
  * **Errors**: `400 Bad Request` (Missing required fields, or a value that fails type/range validation — e.g. a non-numeric `calories`, previously an unhandled case that surfaced as a 500), `401 Unauthorized`, `500 Internal Server Error`.
...
#### `PUT /nutrition-goals`
  ...
  * **Response (200 OK)**:
    ...
    ```
  * **Errors**: `400 Bad Request` (Missing fields, or a value outside the validated range — e.g. calories must fall within 800–4000, protein within 10–500), `401 Unauthorized`, `500 Internal Server Error`.
  * **Note**: The bounds enforced here are defined once and shared with `PUT /profile`, which writes the same underlying column — see Section 5.1 for why that mattered.
...
## 7\. Security Considerations
  ...
  * **Resolved: Nutrition Goal Validation Bypass**: `PUT /nutrition-goals` enforces range checks on submitted values (calories 800–4000, protein 10–500, and similarly for carbs/fat). `PUT /profile` writes the same underlying `nutrition_goals` column but originally applied no validation at all — it serialized whatever was submitted directly into the database. This meant the bounds enforced on the dedicated endpoint could be bypassed entirely by submitting the same data through `/profile` instead, including values outside any sane range or an arbitrarily large nested payload. Fixed by extracting the bounds into a single shared definition used by both endpoints, so they can't drift apart again.
  * **Field Whitelisting on Profile Updates**: `PUT /profile` now rejects unrecognized top-level fields and unrecognized `preferences` fields with a `400` rather than silently ignoring them. An audit at the time confirmed this was not an active privilege-escalation path — no privileged column (e.g. an admin flag or subscription state) was ever settable through this endpoint, and `user_id` is always taken from the verified token rather than the request body — but the whitelist means a future column added to the underlying table isn't automatically mass-assignable by any authenticated client.
  * **Input Validation**: Shared validation helpers (raising a consistent `ValidationError` mapped to `400`) are applied to `PUT /profile`, `PUT /nutrition-goals`, and `POST /food-entries`. Concretely, this closed cases where a non-numeric value for a numeric field (e.g. `calories: "abc"`) previously threw an unhandled exception and surfaced as a `500` rather than a client error, where a range comparison against a string raised a `TypeError` for the same reason, where free-text fields had no length cap, and where a boolean value could pass a naive numeric check (`True == 1`) — the boolean case is now explicitly excluded.
  * **Verified: Parameterized Queries**: The dynamic `UPDATE` statement used for partial profile updates was audited specifically for SQL injection risk, since it assembles its column list dynamically. It builds that list from a fixed set of hardcoded column-name literals (never from client input) and passes every value through parameterized placeholders — confirmed clean, no user-controlled data reaches the query string.
